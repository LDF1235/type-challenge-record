...
type SplitKey<T, Ret extends string[] = []> = T extends `${infer F}.${infer R}`
  ? SplitKey<R, [...Ret, F]>
  : T extends string
  ? [...Ret, T]
  : Ret;

type _Get<T, K, TempObj extends any = T, KeyArr extends string[] = SplitKey<K>> = K extends keyof T
  ? T[K]
  : KeyArr extends [infer F, ...infer R]
  ? F extends keyof TempObj
    ? R extends []
      ? TempObj[F]
      : _Get<T, K, TempObj[F], R extends string[] ? R : []>
    : never
  : never;

type Get<T, K> = _Get<T, K>;
