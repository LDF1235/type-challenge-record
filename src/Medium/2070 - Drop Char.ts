...
type _DropChar<S, C extends string, A extends string = ''> = S extends `${infer F}${infer R}`
  ? _DropChar<R, C, F extends C ? `${A}` : `${A}${F}`>
  : A;

type DropChar<S, C extends string> = _DropChar<S, C>;
