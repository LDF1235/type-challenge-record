...
type GetTargetIndexNumber<T extends number[][], Index extends number> = T extends [infer F, ...infer R]
  ? [F extends number[] ? F[Index] : never, ...GetTargetIndexNumber<R extends number[][] ? R : [], Index>]
  : [];
...
type _Transpose<
  ...
  Ret extends number[][] = []
> = Index extends M[0]['length'] ? Ret : _Transpose<M, PlusOne<Index>, [...Ret, GetTargetIndexNumber<M, Index>]>;

type Transpose<M extends number[][]> = _Transpose<M>;
