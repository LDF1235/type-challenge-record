...
type PickMapFrom<T extends { mapFrom: unknown; mapTo: unknown }, A> = Exclude<
  T,
  Exclude<T, { mapFrom: A; mapTo: any }>
>;

type MapTypes<T, R extends { mapFrom: unknown; mapTo: unknown }> = {
  [P in keyof T]: T[P] extends PickMapFrom<R, T[P]>['mapFrom'] ? PickMapFrom<R, T[P]>['mapTo'] : T[P];
};
