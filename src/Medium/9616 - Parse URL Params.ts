...
type SplitUrlParams<T extends string, Ret extends string[] = []> = T extends `${infer F}/${infer R}`
  ? F extends `:${infer A}`
    ? SplitUrlParams<R, [...Ret, A]>
    : SplitUrlParams<R, Ret>
  : T extends `:${infer B}`
  ? [...Ret, B]
  : Ret;
type ParseUrlParams<T> = T extends string ? SplitUrlParams<T>[number] : never;
