...
type _MergeAll<XS, A extends Record<string, unknown> = {}> = XS extends [infer F, ...infer R]
  ? _MergeAll<
      R,
      {
        [P in keyof A | keyof F]: P extends keyof A
          ? P extends keyof F
            ? A[P] | F[P]
            : A[P]
          : P extends keyof F
...
type MergeAll<XS> = RecordToInterface<_MergeAll<XS>>;

type a = MergeAll<[{ a: 1 | 2 }, { a: 1 | 3 }]>;
