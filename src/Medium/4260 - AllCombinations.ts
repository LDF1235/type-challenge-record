...
type StringToUnion<S, A extends string[] = ['']> = S extends `${infer F}${infer R}`
  ? StringToUnion<R, [...A, F]>
  : A[number];

type KeyUnion = string | number | bigint | boolean | null | undefined;

type _AllCombinations<A extends KeyUnion, B extends KeyUnion = A> = B extends any
  ? B | `${B extends KeyUnion ? B : ''}${_AllCombinations<Exclude<A, B>>}`
  : never;
