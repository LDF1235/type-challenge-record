...
type _PercentageParser<
  A extends string,
  First extends string = A extends `${infer F extends '+' | '-'}${infer R}` ? F : '',
  Last extends string = A extends `${infer F}%` ? '%' : ''
> = [First, A extends `${First}${infer F extends number}${Last}` ? `${F}` : '', Last];

type PercentageParser<A extends string> = _PercentageParser<A>;
