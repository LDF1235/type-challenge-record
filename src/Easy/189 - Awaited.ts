...
type ThenAble = { then: (onfulfilled: (arg: unknown) => unknown) => unknown };

type MyAwaited<T extends Promise<unknown> | ThenAble> = T extends Promise<infer F>
  ? F extends Promise<unknown>
    ? MyAwaited<F>
    : F
  : T extends ThenAble
  ? T['then'] extends (...arg: infer A) => unknown
    ? A[0] extends (...arg: infer B) => unknown
      ? B[0]
      : never
    : never
  : T;
